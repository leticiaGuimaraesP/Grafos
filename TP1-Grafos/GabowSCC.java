...
public class GabowSCC {
    ...
    private int[] id;                // id[v] = id of strong component containing v - Um array para atribuir um identificador único a cada SCC.
    ...
    public GabowSCC(Digraph G) {
        ...
        for (int v = 1; v < G.V(); v++) //inicializa todos as posições do array id com -1
            id[v] = -1;

        //v -> é a posição no array que representa o vertice
        for (int v = 1; v < G.V(); v++) { //aplica a busca em profundidade
            if (!marked[v]) dfs(G, v); //valida se o vértice ja foi visitado 
        }

        // check that id[] gives strong components - validação
        assert check(G);
    }

    private void dfs(Digraph G, int v) {
        marked[v] = true; //vertice visitado
        preorder[v] = pre++; // é uma maneira de rastrear a ordem em que os vértices são visitados durante a bpf
        stack1.push(v);
        stack2.push(v);
